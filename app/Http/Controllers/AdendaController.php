...
class AdendaController extends Controller
{
    ...
    public function create(Request $request): View
    {
        ...
        // ✅ NUEVO: Calcular fechas basadas en adenda anterior o contrato original
        if ($ultimaAdenda) {
            // Si existe adenda anterior, fecha inicio = 1 día después de su fecha fin
            $fechaInicioDefault = $ultimaAdenda->fecha_fin->copy()->addDay();
        } else {
            // Si no existe adenda, fecha inicio = 1 día después de fecha fin del contrato
            $fechaInicioDefault = $contrato->fecha_fin->copy()->addDay();
        }
        ...
        $fechaFinMaxima = $fechaInicioDefault->copy()->addDays($diasDisponibles);

        // Fecha fin = 3 meses después de la fecha inicio (por defecto, pero respetando máximo)
        $fechaFinDefault = $fechaInicioDefault->copy()->addMonths(3);
    ...
    public function generarPDF(Adenda $adenda)
    {
        ...
        if (!$fechaFirma) {
            ...
            $fechaFirma = $adenda->fecha_inicio->copy()->subDay();
        } else {
            // Convertir a Carbon si es string
            $fechaFirma = \Carbon\Carbon::parse($fechaFirma);
        }
    ...
    public function subirAdendaFirmada(Request $request, Adenda $adenda): RedirectResponse
    {
        ...
        // Validar archivo
        $validated = $request->validate([
            'adenda_firmada' => 'required|file|mimes:pdf|max:10240', // 10MB máximo
        ], [
            ...
            'adenda_firmada.mimes' => 'El archivo debe ser PDF',
