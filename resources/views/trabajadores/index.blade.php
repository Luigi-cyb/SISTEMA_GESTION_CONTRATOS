...
@section('content')
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-500 mt-1">
                                Total de registros: <span class="text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100 ml-1">{{ $trabajadores->total() }}</span>
                                @if($estado !== 'Todos')
                                    <span class="mx-2 text-gray-300">|</span>
                                    Estado: <span class="text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-md border border-indigo-100 ml-1">{{ $estado }}</span>
                                @endif
                            </p>
                        </div>
                    </div>
                        @can('create.trabajadores')
            ...
            <div class="bg-white shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    <form method="GET" action="{{ route('trabajadores.index') }}" id="filtrosForm" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                            <!-- Búsqueda por DNI, Nombre, Cargo o Área -->
                            <div class="md:col-span-2">
                                <label for="search" class="block text-sm font-medium text-gray-700 mb-1">
                                    Buscar por DNI, Nombre, Cargo o Área
                                </label>
                                <div class="relative">
                                    ...
                                    <input type="text" name="search" id="search" value="{{ $search }}"
                                        placeholder="Ingrese DNI, nombre..."
                            ...
                            <div>
                                ...
                                <select name="unidad" id="unidad"
                                    ...
                                    @foreach($unidades as $u)
                                        <option value="{{ $u }}" {{ (isset($unidad) && $unidad == $u) ? 'selected' : '' }}>
                                            {{ $u }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="estado" id="estado"
                                    ...
                                    <option value="Suspendido" {{ $estado === 'Suspendido' ? 'selected' : '' }}>Suspendido
                                    </option>
                                    <option value="Todos" {{ $estado === 'Todos' ? 'selected' : '' }}>Ver Todos</option>
                                </select>
                            </div>
                            ...
                            @forelse ($trabajadores as $trabajador)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900">{{ $trabajador->dni }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900">{{ $trabajador->nombre_completo }}</div>
                                        <div class="text-sm text-gray-500">{{ $trabajador->cargo ?? 'Sin cargo' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">{{ $trabajador->unidad }}</div>
                                    </td>
                                    <!-- Indicador de CV -->
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end space-x-3">
                                            <!-- Ver -->
                                            <a href="{{ route('trabajadores.show', $trabajador->dni) }}"
                                            ...
                                            @can('delete.trabajadores')
                                                <form action="{{ route('trabajadores.destroy', $trabajador->dni) }}" method="POST"
                                                    class="inline-block"
                                                    onsubmit="return confirm('¿Está seguro de eliminar este trabajador? Se eliminarán todos sus contratos asociados. Esta acción no se puede deshacer.');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center">
                                        ...
                                        <h3 class="mt-2 text-sm font-medium text-gray-900">No hay trabajadores registrados</h3>
                                        <p class="mt-1 text-sm text-gray-500">Comience creando un nuevo trabajador o cambie el
                                            filtro de estado.</p>
                                        @can('create.trabajadores')
                                            <div class="mt-6">
                                                <a href="{{ route('trabajadores.create') }}"
