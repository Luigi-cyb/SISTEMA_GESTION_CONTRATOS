...
<body>
    ...
    @if($criticos > 0)
        <div class="alert-box">
            <p><strong>ALERTA CRÍTICA:</strong> {{ $criticos }} contrato(s) vencen en 7 días o menos</p>
    ...
    <table>
        ...
        <tbody>
            @foreach($data as $contrato)
                <tr
                    class="{{ $contrato['dias_restantes'] <= 7 ? 'critico' : ($contrato['dias_restantes'] <= 15 ? 'advertencia' : '') }}">
                    <td>{{ $contrato['dni'] }}</td>
                    ...
                    <td>
                        <span
                            class="badge badge-{{ $contrato['dias_restantes'] <= 7 ? 'critico' : ($contrato['dias_restantes'] <= 15 ? 'advertencia' : 'normal') }}">
                            {{ $contrato['dias_restantes'] }} días
                        </span>
                    </td>
                    <td>{{ $contrato['tiempo_formateado'] }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="leyenda">
        ...
        <p>• <strong>[CRÍTICO] ≤ 7 días:</strong> Acción inmediata requerida</p>
        <p>• <strong>[ADVERTENCIA] ≤ 15 días:</strong> Preparar renovación</p>
        <p>• <strong>[NORMAL] > 15 días:</strong> Monitoreo regular</p>
    </div>
