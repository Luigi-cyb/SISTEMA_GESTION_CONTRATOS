            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            <h3 class="text-2xl font-bold text-gray-800">⏰ Contratos Próximos a Vencer</h3>
                            <p class="text-gray-600 mt-1">Contratos que vencen en los próximos <span class="font-semibold">{{ $dias }} días</span> - Total: <span class="font-semibold text-yellow-600">{{ $contratos->count() }}</span></p>
                        </div>
                        <div class="flex space-x-3">
                            <a href="{{ route('reportes.proximos-vencer', ['formato' => 'excel', 'dias' => $dias]) }}" class="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-lg inline-flex items-center transition-colors">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                                </svg>
                                Excel
                            </a>
                            <a href="{{ route('reportes.proximos-vencer', ['formato' => 'pdf', 'dias' => $dias]) }}" class="bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-lg inline-flex items-center transition-colors">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="bg-yellow-50 p-4 rounded-lg mb-6 border border-yellow-200">
                        <form method="GET" action="{{ route('reportes.proximos-vencer') }}" class="flex items-center space-x-4">
                            <label class="text-sm font-medium text-gray-700">Mostrar contratos que vencen en:</label>
                            <select name="dias" class="rounded-lg border-gray-300 focus:border-yellow-500 focus:ring focus:ring-yellow-200" onchange="this.form.submit()">
                    ...
                    <div class="bg-red-50 border-l-4 border-red-500 p-4 mb-6">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-red-700 text-sm">{{ $contratos->where('dias_restantes', '<=', 7)->count() }} contrato(s) vencen en 7 días o menos</p>
                            </div>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50 {{ $contrato['dias_restantes'] <= 7 ? 'bg-red-50' : ($contrato['dias_restantes'] <= 15 ? 'bg-yellow-50' : '') }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{{ $contrato['fecha_fin'] }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $contrato['meses_acumulados'] }} meses</td>
                                </tr>
                                @empty
                                <tr>
                                    <td colspan="9" class="px-6 py-4 text-center text-gray-500">No hay contratos próximos a vencer en los próximos {{ $dias }} días</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
