        ...
        <div class="bg-white shadow-sm sm:rounded-lg mb-6">
            <div class="p-6">
                <div class="flex justify-between items-center">
                    <div>
                        <h1 class="text-2xl font-semibold text-gray-900">Gestión de Contratos</h1>
                        <p class="text-sm text-gray-600 mt-1">Total de registros: <span class="font-medium">{{ $contratos->total() }}</span></p>
                    </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ $contrato->trabajador->nombre_completo }}</div>
                                <div class="text-sm text-gray-500">{{ $contrato->trabajador->cargo ?? 'Sin cargo' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $contrato->fecha_inicio->format('d/m/Y') }}</div>
                                <div class="text-sm text-gray-500">hasta {{ $contrato->fecha_fin->format('d/m/Y') }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2 flex-wrap gap-2">
                                    ...
                                    <!-- ✅ BOTÓN RENOVAR - CONTROLADO POR TIEMPO ACUMULADO -->
                                    @php
                                        $numeroAdendas = \App\Models\Adenda::where('contrato_id', $contrato->id)->count();
                                        // Calcular meses acumulados para bloquear renovación si es >= 59 (4 años 11 meses)
                                        $mesesAcumulados = $contrato->calcularMesesAcumulados();
                                    @endphp
                                    
                                    @if ($contrato->estado === 'Activo')
                                        @if($mesesAcumulados >= 59)
                                            <span class="inline-flex items-center px-2 py-1 bg-red-100 text-red-800 rounded text-xs font-semibold" title="Trabajador próximo a estabilidad laboral ({{ $mesesAcumulados }} meses)">
                                        ...
                                        @else
                                            <!-- Si está ACTIVO y < 59 meses: Botón verde "Renovar" -->
                                        ...
                                        @endif
                                    @elseif ($contrato->estado === 'Vencido')
                                        <!-- Si está VENCIDO: Mostrar adendas -->
                                        <span class="inline-flex items-center px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs font-semibold">
